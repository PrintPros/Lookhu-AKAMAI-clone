...
export function EmbedOptions({ profile }: EmbedOptionsProps) {
  ...
  <iframe 
    src="https://fastfasts-embed-worker.lookhu.workers.dev/?worker=${encodeURIComponent(selectedChannel?.workerManifestUrl?.replace('/index.m3u8', '') || '')}&name=${encodeURIComponent(selectedChannel?.name || 'FastFasts')}&autoplay=${settings.autoPlay ?? true}&muted=${settings.muted ?? true}&controls=${settings.controls ?? true}" 
