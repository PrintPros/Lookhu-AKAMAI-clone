...
export function ChannelManager({ setActiveTab, profile }: ChannelManagerProps) {
  ...
  <iframe 
    src="https://fastfasts-embed-worker.lookhu.workers.dev/?worker=${encodeURIComponent((editingChannel.workerManifestUrl || '').replace('/index.m3u8', '').replace('/live.m3u8', ''))}&name=${encodeURIComponent(editingChannel.name)}&skin=${editingChannel.embedSettings?.skin || 'v1'}&autoplay=${editingChannel.embedSettings?.autoPlay ?? true}&muted=${editingChannel.embedSettings?.muted ?? true}&controls=${editingChannel.embedSettings?.controls ?? true}" 
